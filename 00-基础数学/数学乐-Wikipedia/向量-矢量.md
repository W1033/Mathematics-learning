...
> **矢量 (也说向量)**: 物理学上指由大小和方向共同决定的量。
- 有 **量值 (大小)** 和 **方向**:
- <img src="./img/vector.png" style="display:block; margin-left:0; width:40%;">
...
> **向量的运算**
- 最常用的方法时先把矢量拆分为 $x$ 部分与 $y$ 部分，像这样:<br/> 
...
> **加向量**
- 加向量的做法是: 加 $x$ 部分 和 加 $y$ 部分:
  <img src="./img/addition-vector.png"
        style="display:block; margin-left:0; width:60%;">

> **减向量**
- 先把要减去的向量倒转，然后和另外一个向量相加。
...
- 怎么把 两个向量相乘? 有 2 个方法!
    + **叉积** (也称 向积) [结果是向量]
        ...
        - 两个向量的叉积 ${\bold a} \times {\bold b}$ 是与这 2 个向量垂直的 向量:
           <br/><img src="./img/cross-product.png" 
                style="display:block; margin-left:0; width:30%;">
        - **叉积的运算(1)**: 叉积是这样计算的: ${\bold a} \times {\bold b}$ = 
          ${\bold |a|}{\bold |b|} \sin(\theta){\bold n}$ 
            ...
            + 用图形表示为: 
              <img src="./img/cross-product-operation.png" 
                    style="display:block; margin-left:0; width:36%;">
            + ${\bold n}$ 是 ${\bold a}$ 和 ${\bold b}$ 垂直的 "单位向量"。
                ...
                - 缩放 (Scaling): A vector can be "scaled" off the unit vector.
                  ...
                  Notice they still point in the same direction: <br/>
        ...
        - **叉积的运算(2)**: 我们也可以这样计算叉积: 若 ${\bold a}$ 和 ${\bold b}$ 
...
- (3) 我们可以把向量乘以标量 (叫 "缩放" 向量):
 ...
 - **量值和方向**
    + 我们可能知道向量的量值和方形，但需要它的 $x$ 和 $y$ 长度 (或相反):        
