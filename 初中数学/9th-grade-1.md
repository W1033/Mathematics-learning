    ...
    + **问题(2)**  
      ...
      求每次降价的百分率.
        - **分析:**
            ...
            + (1) 我们先来分析, 每瓶药 56 元, 也就是说: 
              $\color{#E87631}{56(元) \times 1(瓶) = 56 (元/瓶)}$
            + (2) 我们都知道 1 = 100%, 题目中说药品经过两次降价的百分率相同,
              可以理解为就是把 100%(即 1 瓶) 的整体价格下调两次相同的百分比
              (比如: 每次下调 25%);
            + (3) 如果此时设每次降价的百分率为 $x$, 第一次降价后的零售价为: 
              $\color{#E87631}{56(元) \times (1 - x).}$ 
            + (4) 第二次降价是在第一次降价后的基础上(即: 56(1 - $x$) 元) 再降 $x$
              个百分比,  
              即: 第一次降价后的价格 $\times$ (1瓶再次下调 $x$ 个百分比),  
              即: $\color{#E87631}{56(1 - x) \times (1 - x)}$.
        - **解:**
            + 根据上面的分析列出方程:
              $$
                56(1 - x) \times (1 - x) = 31.5.
              $$
